...
std::shared_ptr<DeviceListModel> Controller::model() const
{
    return m_model;
}
    ...
    : QObject(parent)
{
    ...
    m_filter_model = std::make_shared<DeviceFilterProxyModel>(m_model.get(), this);
}
...
void Controller::set_filter_model(const std::shared_ptr<DeviceFilterProxyModel> &filter_model)
{
